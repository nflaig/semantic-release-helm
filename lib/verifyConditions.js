...
module.exports = async (pluginConfig, context) => {
    ...
    if (pluginConfig.registry && !pluginConfig.isChartMuseum && env.REGISTRY_USERNAME && env.REGISTRY_PASSWORD) {
        const registryUrl = pluginConfig.replace("oci://", "").registry.split('/')[0];
        try {
...
async function installCMPushPlugin() {
    await installHelmPlugin('https://github.com/chartmuseum/helm-push');
}

async function installHelmS3Plugin() {
    await installHelmPlugin('https://github.com/hypnoglow/helm-s3.git');
}
